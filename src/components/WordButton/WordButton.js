...
function WordButton({ word, fullCandidateSize }) {
  ...
  return (
    <Toggle
      // className={`${'growShrink'} select-none`}
      className={`${styles.toggle} ${isSelected ? styles.selected : styles.notSelected} growShrink select-none`}
      variant="outline"
      pressed={isSelected}
